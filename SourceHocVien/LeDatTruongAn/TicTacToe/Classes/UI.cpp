...
void UI::ShowGameOver(cocos2d::Layer *layer)
{
	...
	background->setPosition(AnCocos::UI::GetScreenCenter());
	...
	menu->setPosition(Vec2(AnCocos::UI::GetScreenCenter().x, AnCocos::UI::GetScreenCenter().y + screenSize.height));
	layer->addChild(menu);


	EaseBounceOut *menuActionEasing = EaseBounceOut::create(MoveTo::create(MENU_MOVE_BOUNCE_DURATION, AnCocos::UI::GetScreenCenter()));
