...
bool GameScene::init()
{
	...
	AnCocos::UI::AddCentredBackground(GAME_BACKGROUND_FILEPATH, this);

	gridSprite = Sprite::create(GRID_FILEPATH);
	gridSprite->setPosition(AnCocos::UI::GetScreenCenter());
	...
	for (int x = 0; x < 3; x++)
	{
		for (int y = 0; y < 3; y++)
		{
			if (gridSpaces[x][y].containsPoint(touchPoint))
			{
				...
				{
					// AnCocos::GameCenter::unlockAchievement("TTTPlaceFirstButton");
